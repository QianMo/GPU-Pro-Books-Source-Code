...
void OGLES2Skinning::DrawMesh(int i32NodeIndex)
{
	...
	*/
	for (int i32Batch = 0; i32Batch < Mesh.sBoneBatches.nBatchCnt; ++i32Batch)
	{
		...
		*/
		if(Mesh.sBoneIdx.pData && Mesh.sBoneWeight.pData)
		{
			...
			for(int i = 0; i < Mesh.sBoneBatches.pnBatchBoneCnt[i32Batch]; i++)
			{
			...
			glUniformMatrix4fv(m_ShaderProgram.auiLoc[eBoneMatrices], 8, GL_FALSE, amBoneWorld[0].ptr());
			glUniformMatrix3fv(m_ShaderProgram.auiLoc[eBoneMatricesIT], 8, GL_FALSE, afBoneWorldIT[0].ptr());
		}
		else
		{
