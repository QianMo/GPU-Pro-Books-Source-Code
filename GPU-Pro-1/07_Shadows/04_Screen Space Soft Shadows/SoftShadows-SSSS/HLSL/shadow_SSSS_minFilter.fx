...
This implementation uses the min-filter approach for estimating
penumbrae.

The algorithm is performed in some different steps.

1. Compute the shadow map in light space.
     [Pass=MakeShadow]

2. Estimate penumbra using the min-filter approach. As it is a 
     separable kernel, it is done in two different passes.
     [Pass=FattenShadowHoriz;]
	 
3. Estimate penumbra using the min-filter approach (Vertical pass).
	 [Pass=FattenShadowVert;]

4. Compute the hard shadows (standard shadow mapping) for the
...
5. 	Compute the hard shadows for the second visible geometry layer. 
	  (gHardShadowsMapL2).
	  [Pass=MakeHardShadowsL2]
	  
6. Perform the horizontal pass of the Gaussian blur. This step is done
     as a post-processing effect by rendering a quad at full screen.
     [Pass=SmoothShadowHoriz]
	 
7. Perform the vertical pass of the Gaussian blur. This step is done
     as a post-processing effect by rendering a quad at full screen.
     [Pass=SmoothShadowVert]
	 
8. This straightforward step just combines the computed soft shadows 
...
texture gFatShadMap : RENDERCOLORTARGET <
   float2 Dimensions = { SHADOW_SIZE, SHADOW_SIZE };
   string Format = (SHADOW_FMT) ;
   string UIWidget = "None";
>;
...
sampler2D gFatShadSampler = sampler_state {
    texture = <gFatShadMap>;
    AddressU = Clamp;
    AddressV = Clamp;
#if DIRECT3D_VERSION >= 0xa00
...
void makeBuffersAndHardShadowsL1PS(ShadowVertexOutput IN,
	   ...
	   uniform sampler2D FatShadSampler,
...
float4 minFilterPS(JustShadowVertexOutput IN,
	   uniform sampler2D shadMapSampler,
	   uniform float lightSize,
	   uniform float2 horizVert) : COLOR
{
	float kernelWidth = 1.0f;
	float2 sampleStep = kernelWidth*float2(1.0f,1.0f);
	...
	// the following factors affect to the size of the gaussian blur:	
	sampleStep *= horizVert; // select vertical or horizontal pass
	sampleStep *= 0.065*lightSize; // apply Light size (proportional to the size of the penumbra)	
	//0.005f
	float2 offsets[10]={5.0f*sampleStep,4.0f*sampleStep,3.0f*sampleStep,2.0f*sampleStep,sampleStep,
						-5.0f*sampleStep,-4.0f*sampleStep,-3.0f*sampleStep,-2.0f*sampleStep,-sampleStep};
							
	float minz = tex2D(shadMapSampler,IN.LP.xy).x;  // sample the pixel of the center	
	
	for (int i=0; i<10; i++)
		minz = min(minz,tex2D(shadMapSampler,IN.LP.xy+offsets[i]).x);
	
	return float4(minz,minz,minz,1);	
}
...
> {
	   ...
	   VertexShader = compile vs_3_0 quadShadVS();
		...
		CullMode = None;
	   	...
	   	PixelShader = compile ps_3_0 minFilterPS(gShadSampler,
											   gLightSize,
											   float2(1,0));
       }	
	   
       pass FattenShadowVert <
               string Script = "RenderColorTarget0=gFatShadMap;"
			       "RenderDepthStencilTarget=gShadDepthTarget;"
			       "RenderPort=;"
			       "ClearSetColor=gClearColor;"
			       "Draw=buffer;";
	   ...
	   VertexShader = compile vs_3_0 quadShadVS();
		...
		CullMode = None;
	   	...
	   	PixelShader = compile ps_3_0 minFilterPS(gTempMapSampler,
											   gLightSize,
											   float2(0,1));
       }	   
