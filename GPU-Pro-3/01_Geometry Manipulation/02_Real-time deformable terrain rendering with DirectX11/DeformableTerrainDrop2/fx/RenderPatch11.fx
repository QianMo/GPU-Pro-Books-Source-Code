...
#   if USE_TEXTURE_ARRAY
        float Elems;
        g_tex2DMtrlIdxArr.GetDimensions(MtrlIdxTexSize.x, MtrlIdxTexSize.y, Elems);
#   else
        g_tex2DMtrlIdx.GetDimensions(MtrlIdxTexSize.x, MtrlIdxTexSize.y);
#   endif

    float2 TexUVSclaed = In.DiffuseTexUV.xy*MtrlIdxTexSize.xy - float2(0.5, 0.5);
    float2 TexIJ = floor(TexUVSclaed);
    float2 BilinearWeights = TexUVSclaed - TexIJ;
    TexIJ = (TexIJ + float2(0.5, 0.5)) / MtrlIdxTexSize.xy;
    
    float2 Noise = g_tex2DNoiseMap.Sample(samLinearWrap, In.DiffuseTexUV.xy * 50).xy;
    ...
    [unroll]for(int i=-i0; i<=(FilterSize/2); i++)
        ...
        {
            float MtrIdx = SAMPLE_TEX_OR_TEX_ARR_WITH_OFFSET( g_tex2DMtrlIdx, samMaterialIdx, TexIJ.xy, int2(i,j) )*255;
            Colors[i0+i][j0+j] = g_tex2DSrfMtrlArr.Sample(samAnisotropicWrap, float3(In.TileTexUV.xy, MtrIdx) ).rgb;
        }
    
    SurfaceColor = lerp( lerp(Colors[0][0], Colors[1][0], BilinearWeights.x),
...
#       else
            g_tex2DParentDiffuseTex.GetDimensions( ParentDiffuseTexSize.x, ParentDiffuseTexSize.y );
#       endif
