...
Texture2D SpotLightShadowMap : ShadowMaps0;
...
float4 PSMain(Pixel p, uniform bool bShadowed = true) : SV_Target0
{
	...
	float3 toWorld = world - SpotLight.Pos.xyz;
	float3 toWorldDir = normalize(toWorld);

	// Distance falloff
	float distSq = dot(toWorld, toWorld);
	float intensity = 1.0f / (SpotLight.AttenuationOffset + SpotLight.Attenuation * distSq);
	intensity *= saturate(10.0f - 10.0f * distSq / (SpotLight.Range * SpotLight.Range));
	
	// Angle fallof
	intensity *= saturate( dot(eyeGeometry.yzw, -toWorldDir) );

	// Spot falloff
	intensity *= saturate( (dot(toWorldDir, SpotLight.Dir.xyz) - SpotLight.CosOuterAngle) / (SpotLight.CosInnerAngle - SpotLight.CosOuterAngle) );
	...
	return float4(diffuseColor.xyz * SpotLight.Color.xyz * intensity, 0.0f);
}
...
>
{
	pass < string LightType = "SpotLight"; bool Shadowed = false; >
	{
	...
	pass < string LightType = "SpotLight"; bool Shadowed = true; >
	{
		SetVertexShader( CompileShader(vs_4_0, VSMain()) );
		SetGeometryShader( NULL );
