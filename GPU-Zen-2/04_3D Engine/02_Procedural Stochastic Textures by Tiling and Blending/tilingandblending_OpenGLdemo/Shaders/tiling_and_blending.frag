...
uniform vec3 _DXTScalers; // only with DXT compression (Section 1.6)

vec3 ReturnToOriginalColorSpace(vec3 color)
{
	vec3 result = 
		_colorSpaceOrigin +
		_colorSpaceVector1 * color.r +
		_colorSpaceVector2 * color.g +
		_colorSpaceVector3 * color.b;
	return result;
}
...
void main()
{
	color = vec4(ByExampleProceduralNoise(uv * _ScaleTranslate.xy + _ScaleTranslate.zw), 1);
}
