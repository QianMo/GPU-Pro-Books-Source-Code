...
void CEffect::ApplyShaderBlock(SShaderBlock *pBlock, bool resetBufferCounters)
{
    UINT i;

    SD3DShaderVTable *pVT = pBlock->pVT;

	UINT counters[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
	if(!resetBufferCounters)
		for(int i = 0; i < 8; i++)
			counters[i] = D3D11_KEEP_UNORDERED_ACCESS_VIEWS;
    ...
    if( pBlock->UAVDepCount > 0 )
    {
        ...
        {
            m_pContext->CSSetUnorderedAccessViews( pUAVDep->StartIndex, pUAVDep->Count, pUAVDep->ppD3DObjects, counters );
        }
        else
        {
			m_pContext->OMSetRenderTargetsAndUnorderedAccessViews(D3D11_KEEP_RENDER_TARGETS_AND_DEPTH_STENCIL, NULL, NULL, pUAVDep->StartIndex, pUAVDep->Count, pUAVDep->ppD3DObjects, counters);
			
			// FIX FOR PIX
			// m_pContext->OMSetRenderTargetsAndUnorderedAccessViews( 0, NULL, NULL, pUAVDep->StartIndex, pUAVDep->Count, pUAVDep->ppD3DObjects, g_pNegativeOnes );
        }
    }
...
void CEffect::ApplyPassBlock(SPassBlock *pBlock, bool resetBufferCounters)
{
    pBlock->ApplyPassAssignments();
...
#endif
        ApplyShaderBlock(pBlock->BackingStore.pVertexShaderBlock, resetBufferCounters);
    }
...
#endif
        ApplyShaderBlock(pBlock->BackingStore.pPixelShaderBlock, resetBufferCounters);
    }
...
#endif
        ApplyShaderBlock(pBlock->BackingStore.pGeometryShaderBlock, resetBufferCounters);
    }
...
#endif
        ApplyShaderBlock(pBlock->BackingStore.pHullShaderBlock, resetBufferCounters);
    }
...
#endif
        ApplyShaderBlock(pBlock->BackingStore.pDomainShaderBlock, resetBufferCounters);
    }
...
#endif
        ApplyShaderBlock(pBlock->BackingStore.pComputeShaderBlock, resetBufferCounters);
    }
}
