...
public:
  ...
  static OBB2D Create(const int nVertices, const Vec2 *pVertices)
  {
    ...
    float L1 = (-b + d)/2.0f;
    float L2 = (-b - d)/2.0f;
    float Lm = _cpp_max(L1, L2);
    float g1 = CovMat[0] + CovMat[2] - Lm;
    float g2 = CovMat[1] + CovMat[3] - Lm;
    Vec2 Ed = Vec2::Normalize(fabs(g2)>fabs(g1) ? Vec2(1, fabs(g2)>1e-5 ? -g1/g2 : 0) : (Vec2(fabs(g1)>1e-5 ? -g2/g1 : 0, 1)));
