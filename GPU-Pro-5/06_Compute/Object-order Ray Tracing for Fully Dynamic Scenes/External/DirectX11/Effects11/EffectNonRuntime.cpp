...
HRESULT CEffect::BindToDevice(ID3D11Device *pDevice)
{
    ...
    for(; pCB != pCBLast; pCB++)
    {
        ...
        if (pCB->Size > 0)
        {
            // breeze 2.0 MODIFIED: Omit buffer objects that are never used
            ...
            else
            {
                ...
                // breeze 2.0 MODIFIED: Set framework flags
                pCB->IsUserManaged = FALSE;
                pCB->IsSingle = TRUE;
                // MONITOR: Will be reset by Undo***(), currently doesn't matter as user-managed cbufs already taken out of dependency chain
                pCB->IsNonUpdatable = TRUE;
