...
Texture2D<float4> g_DepthBounds                     : register( t2 );
...
{
    // Get scene depth ranges for this tile
    float4 depthBounds = g_DepthBounds.Load(uint3(groupIdx.x, groupIdx.y, 0));
    float minZ = depthBounds.x;
    float maxZ = depthBounds.w;
    float minZ2 = depthBounds.z;
    float maxZ2 = depthBounds.y;
