...
/**************************************************************
** Code to fit my read/write data structs & parameters       **
**    -Chris (11/6/2001)                                     **
**************************************************************/
...
//    { fclose( out ); return GFXIO_BADFILE; }
//  if (RGBE_WritePixels(out,ptr,width*height)!=RGBE_RETURN_SUCCESS)
//    { fclose( out ); return GFXIO_BADFILE; }
//  fclose( out );
//  return GFXIO_OK;
//}
...
static int rgbe_error(int rgbe_error_code, char *msg)
{
  ...
  case rgbe_read_error:
    sprintf(buf, "Error: Read error on RGBE file! '%s'", msg);
    printf( buf );
    break;
  case rgbe_write_error:
    sprintf(buf, "Error: Write error on RGBE file! '%s'", msg);
    printf( buf );
    break;
  case rgbe_format_error:
    sprintf(buf, "Error: Possible bad RGBE format? '%s'", msg);
    printf( buf );
    break;
  default:
  case rgbe_memory_error:
    sprintf(buf, "Error: Memory error in RGBE code! '%s'", msg);
    printf( buf );
  }
