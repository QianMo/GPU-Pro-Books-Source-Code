...
void DoLighting(in StructuredBuffer<LightArrayData> PointLightBuffer, in uint nLightIndex, in float3 vPosition, in float3 vNorm, in float3 vViewDir, out float3 LightColorDiffuseResult, out float3 LightColorSpecularResult)
{
    float4 CenterAndRadius = PointLightBuffer[nLightIndex].v4PositionAndRadius;
    ...
    if( fLightDistance < fRad )
    {
        ...
        LightColorDiffuseResult = PointLightBuffer[nLightIndex].v4Color.rgb * saturate(dot(vLightDir, vNorm)) * fFalloff;

        float3 vHalfAngle = normalize( vViewDir + vLightDir );
        LightColorSpecularResult = PointLightBuffer[nLightIndex].v4Color.rgb * pow(saturate(dot(vHalfAngle, vNorm)), 8) * fFalloff;
    }
}
