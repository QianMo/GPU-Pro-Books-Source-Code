...
public:
	...
	bool parse () 
	{
		...
		_lastTagStart = first;
		_lastTagEnd   = first + (long)endTag.size();
...
public:
	...
	{
		string s(_buffer);
		string::iterator buf = s.begin() + _parseLength;
		return string::iterator(buf);
	}
	...
	long find ( char srchChar, long offset, long length = -1 )
	{
		...
		string s(_buffer);
		string::iterator start = s.begin() + offset;
		string::iterator end   = s.begin() + (offset + length);
	...
	long find ( char * srchStr, long offset, long length = -1 )
	{
		...
		string::iterator srchStart = sSearch.begin();
		string::iterator srchEnd   = sSearch.begin() + strlen(srchStr);
