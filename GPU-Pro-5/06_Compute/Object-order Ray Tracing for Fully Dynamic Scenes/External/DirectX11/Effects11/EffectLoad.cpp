...
HRESULT CEffectLoader::LoadEffect(CEffect *pEffect, CONST void *pEffectBuffer, UINT  cbEffectBuffer)
{
    ...
    // breeze 2.0 MODIFIED: Fix re-invalidated explicit bind points
    for( UINT iVariable=0; iVariable<m_pEffect->m_VariableCount; iVariable++ )
    {
        SGlobalVariable *pVariable = &m_pEffect->m_pVariables[iVariable];
        
        if( pVariable->ExplicitBindPoint == -3 )
            pVariable->ExplicitBindPoint = -1;
    }
...
HRESULT CEffectLoader::LoadObjectVariables()
{
    ...
    for (iBlock=0; iBlock<cBlocks; iBlock++)
    {
        ...
        // breeze 2.0 MODIFIED: Fix up missing bind points
        if (pVar->pSemantic != NULL)
        {
            char *pBindPointSubstr = strstr(pVar->pSemantic, "__bindpoint_");

            if (pBindPointSubstr != NULL)
            {
                char *pRegisterSubstr = pBindPointSubstr + (sizeof("__bindpoint_") - 1);
				// Skip register character
				if (*pRegisterSubstr)
					pVar->ExplicitBindPoint = atoi(pRegisterSubstr + 1);

				int iPreBindPointLength = (int) (pBindPointSubstr - pVar->pSemantic);
				char *pPostBindPointSubstr = strstr(pRegisterSubstr, "__");
				int iPostBindPointLength = (pPostBindPointSubstr) ? strlen(pPostBindPointSubstr) : 0;

				// Remove semantic suffix
				int iFixedSemanticLength = iPreBindPointLength + iPostBindPointLength;
				char *pFixedSemantic = PRIVATENEW char[iFixedSemanticLength + 1];
                memcpy(pFixedSemantic, pVar->pSemantic, iPreBindPointLength);
				if (pPostBindPointSubstr)
					memcpy(pFixedSemantic + iPreBindPointLength, pPostBindPointSubstr, iPostBindPointLength);
                pFixedSemantic[iFixedSemanticLength] = 0;

                m_ReflectionMemory -= AlignToPowerOf2( (UINT)strlen(pVar->pSemantic) + 1, c_DataAlignment)
                    - AlignToPowerOf2( (UINT)iFixedSemanticLength + 1, c_DataAlignment);
                pVar->pSemantic = pFixedSemantic;
            }
        }
...
HRESULT CEffectLoader::GrabShaderData(SShaderBlock *pShaderBlock)
{
    ...
    for(i = 0; i < ShaderDesc.BoundResources; i++)
    {
        ...
        // ASSERT: Validation & assignments only up to this point

        // breeze 2.0 MODIFIED: Allow for unmanaged ('static') resource variables
        if (_strnicmp(ResourceDesc.Name, "Unmanaged_", sizeof("Unmanaged_") - 1) == 0)
        {
            if (pCB)
			{
				// Mark unmanaged
                pCB->ExplicitBindPoint = -2;
				pCB->IsUserManaged = true;
			}
			else if (pVariable)
				// Mark unmanaged
				pVariable->ExplicitBindPoint = -2;
		...
		{
			// breeze 2.0 MODIFIED: Allow for unmanaged ('static') resource variables
			if (pVariable->pSemantic && strstr(pVariable->pSemantic, "__unmanaged"))
			{
				// Mark unmanaged
				pVariable->ExplicitBindPoint = -2;
			...
			{
				// breeze 2.0 MODIFIED: Explicit bind points not properly transferred
				if (pVariable->ExplicitBindPoint == -1)
					pVariable->ExplicitBindPoint = bindPoint;
				if (pVariable->ExplicitBindPoint != bindPoint)
					// Mark ambiguous
					pVariable->ExplicitBindPoint = -3;
			}
		}
    ...
    for (i=0; i<pShaderBlock->UAVDepCount; ++i)
    {
		...
		// breeze 2.0 MODIFIED: Allow for modification of internal counters
		pDep->pInitialCounts = PRIVATENEW UINT[ pDep->Count ];
        ...
        for (j=0; j<pDep->Count; ++j)
        {
            ...
            pDep->ppD3DObjects[j] = NULL;
			// breeze 2.0 MODIFIED: Allow for modification of internal counters
			pDep->pInitialCounts[j] = D3D11_KEEP_UNORDERED_ACCESS_VIEWS;
        }
    }
...
HRESULT CEffectLoader::ReallocateShaderBlocks()
{
    ...
    for (i=0; i<m_pEffect->m_ShaderBlockCount; i++)
    {
        ...
        for (j=0; j<pShader->UAVDepCount; j++)
        {
			...
			// breeze 2.0 MODIFIED: Allow for modification of internal counters
			VHD( pHeap->MoveData((void**) &pUAVDeps->pInitialCounts, pUAVDeps->Count * sizeof(UINT)), pError );
...
UINT  CEffectLoader::CalculateShaderBlockSize()
{
    ...
    for (i=0; i<m_pEffect->m_ShaderBlockCount; i++)
    {
        ...
        for (j=0; j<pShader->UAVDepCount; j++)
        {
			...
			// breeze 2.0 MODIFIED: Allow for modification of internal counters
			dwSize += AlignToPowerOf2(pUAVDeps->Count * sizeof(UINT), c_DataAlignment);
        }
    }
