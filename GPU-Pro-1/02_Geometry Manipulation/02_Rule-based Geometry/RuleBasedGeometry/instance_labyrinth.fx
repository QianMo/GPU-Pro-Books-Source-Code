...
instancingPSInput vsInstance( instancingVSInput input )
{
	...
	// z is inverted to fit the models to the original camera position
	// NOTE: leave the axiom's z at 0, otherwise culling may not work correctly :)
	output.pos.z *= -1;
	output.pos.w = 1;
...
float4 psInstance( instancingPSInput input ) : SV_TARGET
{
	...
	if ( 1 == input.colorID )
	{
		ownColor = txWall.Sample( samLinear, input.tex );
	}
	if ( 2 == input.colorID )
	{
		ownColor = txFloor.Sample( samLinear, input.tex );
	}
