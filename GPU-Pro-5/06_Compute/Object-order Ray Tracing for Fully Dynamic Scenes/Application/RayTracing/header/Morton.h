...
inline uint4 bitzip(const bem::vector<uint4, 2> &v)
{
	return (bitsep1(v[1]) << 1) + bitsep1(v[0]);
}

/// Morton code for 3 dimensions.
inline uint4 bitzip(const bem::vector<uint4, 3> &v)
{
	return (bitsep2(v[2]) << 2) + (bitsep2(v[1]) << 1) + bitsep2(v[0]);
}

/// 2 dimensions from morton code.
inline bem::vector<uint4, 2> bitunzip2(uint4 c)
{
	return bem::vec( bitcomp1(c), bitcomp1(c >> 1) );
}

/// 3 dimensions from morton code.
inline bem::vector<uint4, 3> bitunzip3(uint4 c)
{
	return bem::vec( bitcomp2(c), bitcomp2(c >> 1), bitcomp2(c >> 2) );
}
