...
namespace D3DX11Effects
{
    ...
    const UINT g_pNegativeOnes[8] = { D3D11_KEEP_UNORDERED_ACCESS_VIEWS, D3D11_KEEP_UNORDERED_ACCESS_VIEWS, D3D11_KEEP_UNORDERED_ACCESS_VIEWS,
	...
	ID3D11UnorderedAccessView *const g_pZeroes[8] = { NULL };
...
void CEffect::ApplyShaderBlock(SShaderBlock *pBlock)
{
    ...
    if( pBlock->UAVDepCount > 0 )
    {
        ...
        for (i=0; i<pUAVDep->Count; i++)
        {
			...
			pUAVDep->pInitialCounts[i] = pUAVDep->ppFXPointers[i]->InitialCount;
        }

        if( EOT_ComputeShader5 == pBlock->GetShaderType() )
        {
			// breeze 2.0 MODIFIED: Allow for modification of internal counters
            m_pContext->CSSetUnorderedAccessViews( pUAVDep->StartIndex, pUAVDep->Count, pUAVDep->ppD3DObjects, pUAVDep->pInitialCounts );
        }
        else
        {
			...
			// breeze 2.0 MODIFIED: Allow for modification of internal counters
            m_pContext->OMSetRenderTargetsAndUnorderedAccessViews( D3D11_KEEP_RENDER_TARGETS_AND_DEPTH_STENCIL, NULL, NULL, pUAVDep->StartIndex, pUAVDep->Count, pUAVDep->ppD3DObjects, pUAVDep->pInitialCounts );
        }
    }
...
// breeze 2.0 MODIFIED
void CEffect::UnbindShaderBlock(SShaderBlock *pBlock)
{
	SD3DShaderVTable *pVT = pBlock->pVT;
    ...
    if( pBlock->UAVDepCount > 0 )
    {
        ...
        {
			// breeze 2.0 MODIFIED: Allow for modification of internal counters
            m_pContext->CSSetUnorderedAccessViews( pUAVDep->StartIndex, pUAVDep->Count, g_pZeroes, g_pNegativeOnes );
        }
        else
        {
			// This call could be combined with the call to set render targets if both exist in the pass
			...
			// breeze 2.0 MODIFIED: Allow for modification of internal counters
            m_pContext->OMSetRenderTargetsAndUnorderedAccessViews( D3D11_KEEP_RENDER_TARGETS_AND_DEPTH_STENCIL, NULL, NULL, pUAVDep->StartIndex, pUAVDep->Count, g_pZeroes, g_pNegativeOnes );
        }
    }
}
...
void CEffect::ApplyPassBlock(SPassBlock *pBlock)
{
    ...
    if (NULL != pBlock->BackingStore.pRenderTargetViews[0])
    {
		...
		// breeze 2.0 MODIFIED: Fixed uav start offset
        m_pContext->OMSetRenderTargetsAndUnorderedAccessViews(
			pBlock->BackingStore.RenderTargetViewCount, pRTV, pBlock->BackingStore.pDepthStencilView->pDepthStencilView,
			pBlock->BackingStore.RenderTargetViewCount, D3D11_KEEP_UNORDERED_ACCESS_VIEWS, NULL, NULL );
    }
...
// breeze 2.0 MODIFIED
void CEffect::UnbindPassBlock(SPassBlock *pBlock)
{
    if (NULL != pBlock->BackingStore.pVertexShaderBlock)
        UnbindShaderBlock(pBlock->BackingStore.pVertexShaderBlock);

    if (NULL != pBlock->BackingStore.pPixelShaderBlock)
        UnbindShaderBlock(pBlock->BackingStore.pPixelShaderBlock);

    if (NULL != pBlock->BackingStore.pGeometryShaderBlock)
        UnbindShaderBlock(pBlock->BackingStore.pGeometryShaderBlock);

    if (NULL != pBlock->BackingStore.pHullShaderBlock)
        UnbindShaderBlock(pBlock->BackingStore.pHullShaderBlock);

    if (NULL != pBlock->BackingStore.pDomainShaderBlock)
        UnbindShaderBlock(pBlock->BackingStore.pDomainShaderBlock);

    if (NULL != pBlock->BackingStore.pComputeShaderBlock)
        UnbindShaderBlock(pBlock->BackingStore.pComputeShaderBlock);
}
