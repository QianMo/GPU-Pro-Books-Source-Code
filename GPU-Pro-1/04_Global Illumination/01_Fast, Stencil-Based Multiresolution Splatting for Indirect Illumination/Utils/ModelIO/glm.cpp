...
/* glmGetIndex: face indices (either "v", "vn" or "vt") can either be 
 * absolute or relative.  IndexVal > 0 implies an absolute index, 
 * IndexVal < 0 implies a relative index (count back from current location)
 * IndexVal == 0 should throw an error, since it is illegal.
 *
 * relativeFileIndex    -> the index (v/vn/vt) from file
 * currentPropertyCount -> the current count of that property at current location in file.
 */
static GLuint glmGetIndex( int relativeFileIndex, GLuint currentPropertyCount )
{
	return relativeFileIndex >= 0 ? relativeFileIndex : currentPropertyCount+relativeFileIndex;
}
...
glmSecondPass(GLMmodel* model, FILE* file) 
{
  ...
  Material* fwMaterial=0;         /* current material */
  GLuint    v, n, t;
  char      buf[128];
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'u':
	  ...
	  group->fwMaterial = fwMaterial = model->fwScene->ExistingMaterialFromFile( buf );
	  if (!group->fwMaterial)
		  printf("Warning:  Unable to locate scene material '%s' referenced in .obj file!\n", buf);
      break;
    case 'g':				/* group */
      /* eat up rest of line */
      fgets(buf, sizeof(buf), file);
	...
	/* v//n */
	sscanf(buf, "%d//%d", &v, &n);
	T(numtriangles).vindices[0] = glmGetIndex(v,numvertices);
	T(numtriangles).nindices[0] = glmGetIndex(n,numnormals);
	fscanf(file, "%d//%d", &v, &n);
	T(numtriangles).vindices[1] = glmGetIndex(v,numvertices);
	T(numtriangles).nindices[1] = glmGetIndex(n,numnormals);
	fscanf(file, "%d//%d", &v, &n);
	T(numtriangles).vindices[2] = glmGetIndex(v,numvertices);
	T(numtriangles).nindices[2] = glmGetIndex(n,numnormals);
	...
	/* v/t/n */
	T(numtriangles).vindices[0] = glmGetIndex(v,numvertices);
	T(numtriangles).tindices[0] = glmGetIndex(t,numtexcoords);
	T(numtriangles).nindices[0] = glmGetIndex(n,numnormals);
	fscanf(file, "%d/%d/%d", &v, &t, &n);
	T(numtriangles).vindices[1] = glmGetIndex(v,numvertices);
	T(numtriangles).tindices[1] = glmGetIndex(t,numtexcoords);
	T(numtriangles).nindices[1] = glmGetIndex(n,numnormals);
	fscanf(file, "%d/%d/%d", &v, &t, &n);
	T(numtriangles).vindices[2] = glmGetIndex(v,numvertices);
	T(numtriangles).tindices[2] = glmGetIndex(t,numtexcoords);
	T(numtriangles).nindices[2] = glmGetIndex(n,numnormals);
	group->triangles[group->numtriangles++] = numtriangles;
	numtriangles++;
	while(fscanf(file, "%d/%d/%d", &v, &t, &n) > 0) {
 ...
 */
GLMmodel* 
glmReadOBJ(char* filename, Scene *s)
{
  GLMmodel* model;
  FILE*     file;
...
GLvoid
glDrawOneGroup(GLMmodel* model, GLuint mode, GLMgroup *grp)
{
  static GLuint i;
  static GLMgroup* group;
  static GLMtriangle* triangle;
...
GLuint
glmGroupList(GLMmodel* model, GLuint mode, GLMgroup* grp)
{
  GLuint list;

  list = glGenLists(1);
  glNewList(list, GL_COMPILE);
  glDrawOneGroup(model, mode, grp);
  glEndList();
