...
public:
  ...
  inline bool IsIntersecting(const Mat4x4& OBB)
  {
    ...
    Mat4x4 iMVPiOBB = m_InvMVP*OBBInverse(OBB);
    for(int i=0; i<8; ++i)
    {
...
protected:
  ...
  static Mat4x4 OBBInverse(const Mat4x4& SRT)
  {
    Mat4x4 SR = SRT;
    SR.r[3] = c_WAxis;
    Mat4x4 tmp = Mat4x4::Transpose(SR);
    Vec4 r0 = tmp.r[0];
    Vec4 r1 = tmp.r[1];
    Vec4 r2 = tmp.r[2];
    Vec4 r3 = tmp.r[3];
    Vec4 r = Vec4::Rcp((r0*r0 + r1*r1 + r2*r2) | r3);
    Mat4x4 iRiS;
    iRiS.r[0] = r.x*Vec4(SR.r[0]);
    iRiS.r[1] = r.y*Vec4(SR.r[1]);
    iRiS.r[2] = r.z*Vec4(SR.r[2]);
    iRiS.r[3] = r3;
    iRiS = Mat4x4::Transpose(iRiS);
    Mat4x4 iTiRiS = iRiS;
    iTiRiS.r[3] = Vec3::Point(-Vec3(SRT.r[3]))*iRiS;
    return iTiRiS;
  }
