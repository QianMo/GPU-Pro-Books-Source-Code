...
void depthSearch(inout LinkedListEntryWithPrev entry, inout LinkedListEntryNeighbors entryNeighbors, float z, out float outShading)
{
...
float4 ps_main(PS_IN input) : SV_Target
{
	...
	for(int x = 0; x < FILTER_SIZE * 2 + 1; x++)
	{
		...
		for(int y = 0; y < FILTER_SIZE * 2 + 1; y++)
		{
			...
			depthSearch(currentYEntry, currentYEntryNeighbors, posInLight.z, localShading);
