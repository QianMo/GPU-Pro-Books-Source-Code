/*!****************************************************************************
 @File          OGLES2IntroducingPODTouch.cpp
 ...
 @Description   Shows how to load POD files and play the animation
				with basic lighting
******************************************************************************/
#define TOUCH_ENABLE
#include <string.h>
...
const float g_fDemoFrameRate = 1000.0f / 30.0f;
...
******************************************************************************/
class OGLES2IntroducingPODTouch : public PVRShell
{
	...
	// 3D Model
#ifdef TOUCH_ENABLE
	MDK::Input::TouchDevice	m_mdkTouch;
	MDK::Common::PrecisionTimer m_mdkTimer;
#endif
...
******************************************************************************/
bool OGLES2IntroducingPODTouch::LoadTextures(CPVRTString* pErrorStr)
{
/*
		Loads the textures.
...
******************************************************************************/
bool OGLES2IntroducingPODTouch::LoadShaders(CPVRTString* pErrorStr)
{
	/*
...
******************************************************************************/
bool OGLES2IntroducingPODTouch::LoadVbos(CPVRTString* pErrorStr)
{
...
/*!****************************************************************************
 ...
 @Description	Code in InitApplication() will be called by PVRShell once per
				...
				not be called again.
******************************************************************************/
bool OGLES2IntroducingPODTouch::InitApplication()
{
	m_puiVbo = 0;
	m_puiIndexVbo = 0;
	m_puiTextureIDs = 0;
...
/*!****************************************************************************
 ...
 @Description	Code in QuitApplication() will be called by PVRShell once per
				...
				not be called.
******************************************************************************/
bool OGLES2IntroducingPODTouch::QuitApplication()
{
...
******************************************************************************/
bool OGLES2IntroducingPODTouch::InitView()
{
	CPVRTString ErrorStr;
#ifdef TOUCH_ENABLE

	m_bRotate = PVRShellGet(prefIsRotated);// && PVRShellGet(prefFullScreen);
	m_mdkTouch.Init(this, PVRShellGet(prefWidth), PVRShellGet(prefHeight),m_bRotate);

	m_mRotation = PVRTMat4::Identity();
	m_bSaveRot = true;

#ifdef WIN32
	if(PVRShellGet(prefFullScreen))
		ShowCursor(0);
#endif
#endif
	/*
...
******************************************************************************/
bool OGLES2IntroducingPODTouch::ReleaseView()
{
...
******************************************************************************/
bool OGLES2IntroducingPODTouch::RenderScene()
{
	// Clear the color and depth buffer
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	if(PVRShellIsKeyPressed(PVRShellKeyNameSELECT))
		m_bSaveRot = !m_bSaveRot;
	...
	if (m_mdkTouch.IsReleased())
	{
		m_fReleaseTime = m_mdkTimer.GetTimef();
	}


	float releaseMultiplier = 0.005;
	float dragMultiplier = 0.03;

	if(!m_bSaveRot)
	{
		releaseMultiplier = 0.02;
		dragMultiplier = 0.1;
	}

	float amplitudeX = m_mdkTouch.GetReleaseMotionX() * releaseMultiplier;
	float amplitudeY = m_mdkTouch.GetReleaseMotionY() * releaseMultiplier;
	float tau = 7.0f;

	m_X += MDK::Math::DerivativeExp(m_mdkTimer.GetTimef() - m_fReleaseTime, amplitudeX, tau);
	m_X += m_mdkTouch.GetMotionX() * dragMultiplier;

	m_Y += MDK::Math::DerivativeExp(m_mdkTimer.GetTimef() - m_fReleaseTime, amplitudeY, tau);
	m_Y += m_mdkTouch.GetMotionY() * dragMultiplier;
	...
	*/
	...
	m_fFrame += m_mdkTimer.GetDeltaTimef() * g_fDemoFrameRate;
	...
	*/
...
#ifdef TOUCH_ENABLE
		PVRTMat4 mRot = PVRTMat4::RotationX(-m_Y) *PVRTMat4::RotationY(-m_X);
		if(m_bSaveRot)
			m_mRotation = mRot * m_mRotation;
		else
			m_mRotation = mRot;
		mWorld = m_mRotation * mWorld;
#endif
	...
	//m_Print3D.Print3D(80,10,0.5, m_bSaveRot? 0xFF00FFFF : 0xFF0000FF, "X = %0.2f\nY = %0.2f", m_X, m_Y);
	if(m_bSaveRot)
		m_X = m_Y = 0;
	// Displays the demo name using the tools. For a detailed explanation, see the training course IntroducingPVRTools
	m_Print3D.DisplayDefaultTitle("IntroducingTouchscreen", m_bSaveRot ? "Rotate around object" : "Rotate around axis", ePVRTPrint3DLogoIMG);
	m_Print3D.Flush();
...
******************************************************************************/
void OGLES2IntroducingPODTouch::DrawMesh(int i32NodeIndex)
{
...
PVRShell* NewDemo()
{
	return new OGLES2IntroducingPODTouch();
}

/******************************************************************************
 End of file (OGLES2IntroducingPODTouch.cpp)
******************************************************************************/
