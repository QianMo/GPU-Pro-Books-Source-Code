...
static HRESULT CreateTextureFromDDS( _In_ ID3D12Device* d3dDevice,
                                     ...
                                     _In_ bool forceSRGB,
                                     _Outptr_opt_ ID3D12Resource** texture,
                                     _Outptr_opt_ ID3D12Resource** uploadBuffer,
                                     _In_ D3D12_CPU_DESCRIPTOR_HANDLE textureView )
{
    HRESULT hr = S_OK;
...

    {
        ...
        if ( SUCCEEDED( hr ) )
        {
            ...
            d3dCommandList->ResourceBarrier( 1, &CD3DX12_RESOURCE_BARRIER::Transition( *texture, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST ) );
            UpdateSubresources( d3dCommandList, *texture, *uploadBuffer, 0, 0, subresourceCount, initData.get() );
            d3dCommandList->ResourceBarrier( 1, &CD3DX12_RESOURCE_BARRIER::Transition( *texture, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_GENERIC_READ ) );
        }
    }
    ...
    DDS_ALPHA_MODE* alphaMode )
{
    ...
    HRESULT hr = CreateTextureFromDDS( d3dDevice, d3dCommandList,
                                       header, ddsData + offset, ddsDataSize - offset, maxsize,
                                       forceSRGB, texture, uploadBuffer, textureView );
    if ( SUCCEEDED( hr ) )
    {
    ...
    DDS_ALPHA_MODE* alphaMode = nullptr )
{
    ...
    hr = CreateTextureFromDDS( d3dDevice, d3dCommandList,
